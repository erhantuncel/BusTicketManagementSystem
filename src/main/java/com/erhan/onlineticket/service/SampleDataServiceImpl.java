...
import com.erhan.onlineticket.model.Gender;
...
//	protected Long[] ticketIds;
	...
	public void populateData() {
		...
		Customer adminAccount = new Customer();
		adminAccount.setTcNumber("10987654321");
		adminAccount.setName("Ahmet");
		adminAccount.setSurname("ÇALIŞKAN");
		adminAccount.setGender(Gender.ERKEK);
		adminAccount.setDateOfBirth(generateRandomDateMaximum18Year());
		adminAccount.setMobileNumber(generateRandomGsmNumber());
		adminAccount.seteMail(generateNameForEmail("ahmet") + "@abc.com");
		String adminPassword = "admiN123";
		adminAccount.setPassword(passwordEncoder.encode(adminPassword));
//		adminAccount.setPassword("admiN123");
		adminAccount.setDateOfRegister(generateCustomerRegisteredTime(180, 150));
		adminAccount.setTimeOfLastOnline(generateCustomerLastOnlineTime(16, 0));
		adminAccount.setEnabled(true);
//		customerService.create(adminAccount);
		userRoleAdmin.setUser(adminAccount);
		userRoleService.create(userRoleAdmin);
		logger.info("Admin Account(TcNumber: " + adminAccount.getTcNumber() + ", Password: " + adminPassword + " ) is created.");

		// Creating User account
		UserRole userRoleUser = new UserRole("ROLE_USER");
		Customer userAccount = new Customer();
		userAccount.setTcNumber("12345678910");
		userAccount.setName("Mehmet");
		userAccount.setSurname("YILMAZ");
		userAccount.setGender(Gender.ERKEK);
		userAccount.setDateOfBirth(generateRandomDateMaximum18Year());
		userAccount.setMobileNumber(generateRandomGsmNumber());
		userAccount.seteMail(generateNameForEmail("mehmet") + "@abc.com");
		String userPassword = "useR1234";
		userAccount.setPassword(passwordEncoder.encode(userPassword));
//		userAccount.setPassword("useR1234");
		userAccount.setDateOfRegister(generateCustomerRegisteredTime(180, 150));
		userAccount.setTimeOfLastOnline(generateCustomerLastOnlineTime(16, 0));
		userAccount.setEnabled(true);
//		customerService.create(userAccount);
		userRoleUser.setUser(userAccount);
		userRoleService.create(userRoleUser);
		logger.info("User Account(TcNumber: " + userAccount.getTcNumber() + ", Password: " + userPassword + ") is created.");
...
//			logger.info("departure date = {} diff = {}", df.format(departureTime.getTime()), String.valueOf(differenceOfDepartureAndToday));
			...
			while (extraPassengerCountTemp <= extraPassengerCount) {
				...
				if (!isMaxCountReachForDepartureAndArrival(departureIndex, arrivalIndex, ticketList, seatCount,
					...
					if (voyageNumber % 10 == 0 && ticketForTestCustomer) {
						...
						userAccount.setTimeOfLastOnline(generatedDateForLastOnline);
						userSerhanLastOnlineDates.add(generatedDateForLastOnline);
						customerService.update(userAccount);
						ticket = createTicketForTestCustomer(userAccount, voyage, stops, seatCount, ticketList,
		...
		userAccount.setTimeOfLastOnline(lastOnlineForUserSerhan);
		customerService.update(userAccount);
		...
		for (Voyage voyage : voyageList) {
			...
			if(calculateDifferenceBetweenDatesInSecond(now, expenseTime) < 0) {				
