										...
										<tr>
											<c:forEach var="j" begin="0" end="2">
												...
												<c:choose>
													...
													<c:otherwise>
														<c:choose>
															<c:when test="${empty ticketArray[i][j]}">
																<td class="${(j==0)?'singleSeat':'doubleSeat'}" data-col="${j+1}">
																	&nbsp;<input type="checkbox" value="${number}">&nbsp;&nbsp;&nbsp;${number}
																</td>
															</c:when>
															<c:otherwise>	
																<c:set var="gender" value="${(ticketArray[i][j].passangerGender == 'ERKEK')?'male':'female'}"/>
																<td class="${(j==0)?'singleSeat':'doubleSeat'}" data-col="${j+1}" data-gender="${gender}">
																	<img src="<c:url value="/resources/icon/${gender}-icon-24.png"/>">&nbsp;${number}
																</td>
															</c:otherwise>
														</c:choose>
